...
set "MD=C:\Users\jonat\Desktop\CoyoteThings\RealityScan_CLI\modules\realityscan_interface\RS_CLI\Metadata"
...
- **Never pass delimited data as a .bat argument.** cmd splits unquoted `;` `,` `=` into separate
  ...
  [VERIFIED: NA167 B5; CLAUDE.md hard rule 8]
...
|---|---|---|---|
| `MvsExportcoordinatesystemtype` *(sic — lowercase `c`)* | int | `0`, `3` | export CRS mode. The dialog's *Coordinate system* options, in Help order: **Grid plane** (as seen in the 3D view) / **Project Output** (georeferenced models only; set in WORKFLOW ▸ Settings ▸ Coordinate systems) / **Shifted project output** (Project Output moved to the scene centre) / **Same as XMP** (also available for non-georeferenced models) [OFFICIAL: tools/export]. **`3` = Same as XMP = GEOCENTRIC (EPSG:4978) geometry with identity `transformToModel`**, and `globalCoordinateSystem` in that case is a stale or arbitrary label, not a description of the data — 300,000 exported vertices read as ECEF land on the dive site; read as the declared 55N they land in the Indian Ocean [VERIFIED: FINDINGS 2026-09-01/02]. `0`..`2` indexing the list in order remains [INFERRED], consistent with `1` (LAS, identity) and `2` (OBJ, local frame + transform) as observed. |
...
[OFFICIAL: install `sceneobjects.xml`; CLAUDE.md naming exception]
...
| `exportCoordinateSystemType` | observed `1` (LAS, identity transform), `2` (OBJ, local frame), and — since 2026-09-01 — `3` (OBJ/FBX from `ExportDeliverables.bat`: **ECEF geometry, identity transform, arbitrary `globalCoordinateSystem` label**) [VERIFIED: FINDINGS 2026-09-01]. `0` (PLY) is still unobserved |
...
All at `C:\Users\jonat\Desktop\CoyoteThings\RealityScan_CLI\modules\realityscan_interface\RS_CLI\Metadata\`.
...
| `SimplifySmooth_80per_Params.xml` | simplify | `run_decimate.py` (the 500k web tier) — `GenerateModel.bat` step [7/8] ran it 4× until 2026-09-06 | relative 80 %, otherwise identical | **Production** (decimate lane) — same placeholder caveat |
| `Simplify75per_Params.xml` | simplify | `GenerateModel.bat` step [7/8] and `ModelToFinal.bat`, looped on the measured count until at or under `RS_TARGET_TRIS` (10 M) — D12, 2026-09-06 | relative 75 % kept per pass, otherwise identical to the 80 % file | **Production** |
...
| 16 | `MvsExportcoordinatesystemtype`: does `0..3` index Grid plane / Project Output / Shifted project output / Same as XMP? **RESOLVED for `3` (2026-09-01): Same as XMP = ECEF geometry, identity transform.** `0`..`2` still inferred | export the same model at Grid plane / Project Output / Shifted and diff the profiles against the observed `1` and `2` | minutes, needs the GUI |
