...
def parse_arguments(argv, params, logger) -> None:
    ...
    for p in params.values():
        ...
        if val is None and p.prompt_user:
            # _default_for, NOT get. This is a PROMPT default - precisely the
            # case RS_NO_SETTINGS_INHERITANCE exists to refuse - but it read
            # the store ungated, so the strict agent lane never reached the
            # orchestrator's own parameters. Every module knob (zone sizes,
            # prior accuracies, output paths) inherited the previous
            # campaign's answers even on a lane that had explicitly refused
            # inheritance, which is the wizard-prefill incident class the flag
            # was written for. Same fix as BatchDirectory._stored_default.
            # Duck-typed for SettingsStore-shaped test doubles, which are only
            # required to provide `get`.
            _gated = getattr(settings, '_default_for', None)
