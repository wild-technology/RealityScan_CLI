...
| B11 | Product fix queued: `run_attach_script` should clear the OWN instance's ErrorWriter markers (`errors_`/`results_`, never `progress_` — held open) when `instance == self.instance_name`, matching run_batch_script and the .bat gate's own/foreign split. Until then, own-instance attach after a failed run needs a manual marker clear. Plus UX: `export_deliverables.py` prompted for `cache_dir` mid-run (EOF-fallback saved it); unattended drivers should not prompt when env/settings can answer. | OPEN |
...
| B12 | CLAUDE.md staleness introduced by consolidation step 8, un-fixable locally while the stranded rewrite owns the file: the standalone-scripts list (CLAUDE.md ~line 107) still names `masking.py`, renamed `timestamp_rename.py` 2026-08-07 (it was always a timestamp renamer, never a masker). Fold the rename into the incoming CLAUDE.md rewrite when it lands. | BLOCKED(push) |
