...
| A7 | Their CLAUDE.md rewrite: read-order, pytest baseline, nine working practices, session-end checklist, architecture brought current | BLOCKED(push) — integrate by rebase when it lands; local `main` (16+ commits) edits CLAUDE.md and WILL conflict with a full rewrite. |
...
| B12 | CLAUDE.md staleness introduced by consolidation step 8, un-fixable locally while the stranded rewrite owns the file: the standalone-scripts list (CLAUDE.md ~line 107) still names `masking.py`, renamed `timestamp_rename.py` 2026-08-07 (it was always a timestamp renamer, never a masker). ALSO stale in frozen CLAUDE.md (~82–83): `AlignImageList` and `SequentialAlignGrow` listed as live workflows, both owner-retired to `archive/legacy_scripts/` 2026-08-07. Fold both into the incoming CLAUDE.md rewrite when it lands. | BLOCKED(push) |
