...
| B4 | `ModelToFinal.bat` current head has never run live: 80% simplify + 4x8k default (owner 2026-08-07), `objmetric` path, caller-captured `RS_INSTANCE`, marker gate keyed on `RS_TARGET==RS_INSTANCE`, explicit re-select before rename. Needs one attach-mode smoke against a small scene (known-good) and one deliberate-failure probe (known-bad) per the oracle rule. | OPEN |
...
| # | Item | Status |
|---|---|---|
| C1 | **Euler-order / camera-mount pin** (`ifKGrp`/`ifKmode` value mapping undocumented; both fall to defaults). THE standing gate: no further orientation cell until pinned. Two settle paths already designed (GUI dropdown diff · headless ifKmode sweep reading pose XMPs). | OPEN — highest-value single verification in the repo |
...
| C5 | `lastError` stickiness scope: verified idle-stickiness and clear-on-next-op for ONE code (0x82010061 after failed save). Unverified: whether all error classes behave identically, and whether `rev` increments on FAILED operations (the `:run` gate assumes it does). | OPEN |
