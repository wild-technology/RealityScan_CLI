...
| B9 | Live gate for `run_attach_script` / `finish_model.py`: one real attach run against a loaded scene (boot RS1, load the mtf battery scene, `python finish_model.py --instance RS1 --outdir ...`), expecting the same artifact census as the 4b battery cell. Offline stub tests pass (7); the Python layer has not yet driven a real instance. | OPEN |
| B10 | Live gate for the rerouted export: one `ExportDeliverables.bat` run through `modules/export_deliverables.py` on a modelled workspace — verify lock file, cleared markers, resource CSV, and that a pre-planted stale `errors_RS1.txt` is cleared pre-run (run_batch_script semantics). | OPEN |
